...
def create_distance_matrix(sequences: list, is_lower_triangle=False):
...
def create_triangle_distance_matrix(dist_matrix):
    res = []
...
if __name__ == '__main__':
    ...
    singles_seqs = df_singletons_merged['Peptide'].values
    with open('singletons_reverse.txt', 'w') as fout:
        for s in singles_seqs:
            fout.write(s[::-1] + "\n")



    # fetch all cluster subsequences
    cluster_subsequences = df_merged['subseqs'].explode()
    cluster_subsequences = [s for s in cluster_subsequences if len(s) == 21]

    # print to fasta file
    with open('for_iedb_2620.fasta', 'w') as fout:
    ...
    all_sequences = cluster_subsequences
    print('computing dist matrix...')

    create_new_mat = False
    M_file = './dist_matrix_square.pickle'
    if create_new_mat:
        dist_matrix_square = create_distance_matrix(all_sequences, is_lower_triangle=False)  # now creates the entire matrix
        with open(M_file, 'wb') as pickle_file:
            pickle.dump(dist_matrix_square, file=pickle_file)
    else:
        with open(M_file, 'rb') as pickle_file:
            dist_matrix_square = pickle.load(file=pickle_file)

    dist_lists = create_triangle_distance_matrix(dist_matrix_square)
    ...
    if False:
        ...
        tree1 = constructor.upgma(DistanceMatrix(names=all_sequences, matrix=dist_lists))
        Bio.Phylo.draw(tree1)

    # Create a tree 2
    # print('Constructing tree using treecluster')
    # remove last 0 for each list
    # dist_lists = [ sub_lst[:-1] for sub_lst in dist_lists]
    # tree2 = Bio.Cluster.treecluster(data=None, distancematrix=dist_lists[:1000])

    # Create a tree 3:
    D = dist_matrix_square
    ...
    idx1 = Z1['leaves']
    D2 = D[idx1, :]
    im = axmatrix.matshow(D2, aspect='auto', origin='lower', cmap=pylab.cm.YlGnBu)
    axmatrix.set_xticks([])
    axmatrix.set_yticks([])
    ...
    all_sequences_new_order = [all_sequences[idx1[i]] for i in range(0, len(idx1))]
    ax2.set_xticklabels(all_sequences_new_order)
    # ax2.set_yticklabels(all_sequences_new_order)
    plt.rcParams.update({'font.size': 22})
    plt.show()

    # TEST:
    wanted = ["QLYFFSSLVCTECVLLASMAY", "QLYFFHFLGSTECFLYTVMSY", "QMYFFFFFGVAECFLLATMAY", "QMFFFLFFGATECCLLAAMAY",
              "QMFFFVFLGAAECFLLSSMAY", "QMFFFVTLGSTDCFLLAIMAY", "QLYFFHFLGSTQCFLYTLMAY",
              "QLYFFLGLGCTECVLLAVMAY", "QMYFFFFFGAAECCLLATMAY", "QIYFFHSLGATECYLLTAMAY", "QLYFFIALACTECVLLAVMAY",
              "QLYFFIALMCTECVLLAAMAY", "QLYFFVFLGATECFLLAFMAY", "QLFIFTFLGATECFLLAAMAY"]

    # find the position of eact wanted sequence in all_sequences_new_order
    wanted_idx = []
    for wanted_seq in wanted:
        wanted_idx.append(all_sequences_new_order.index(wanted_seq))
    print(wanted_idx)

    for i in range(196, 285):
        if i in wanted_idx:
            print('*' + all_sequences_new_order[i] + "*")
        else:
            print(all_sequences_new_order[i])


    with open('test2600.txt', 'w') as fout:
        for seq in all_sequences_new_order:
            fout.write(seq + "\n")
